#!/usr/bin/env bash
# Sign every cks/*.iso object in the bucket with the Atlas Cloud artifact key
...
# BUCKET_NAME (see scripts/export-atlas-s3-env.sh).
# Optional env:
...
aws "${aws_opts[@]}" s3api list-objects-v2 \
    --bucket "$BUCKET_NAME" --prefix "cks/" \
    --output json \
  | jq -r '.Contents[]?.Key | sub("^cks/"; "")' > "$tmp/keys"

mapfile -t isos < <(grep -E '\.iso$' "$tmp/keys" || true)
if [[ ${#isos[@]} -eq 0 ]]; then
  echo "[sign] no ISOs found in s3://${BUCKET_NAME}/cks/"
  exit 0
fi
...
for iso in "${isos[@]}"; do
  ...
  gpg "${gpg_args[@]}" \
      ...
      "$tmp/download/${iso}"
  aws "${aws_opts[@]}" s3 cp "$tmp/download/${asc}" "s3://${BUCKET_NAME}/cks/${asc}" \
...
for minor in "${!minor_isos[@]}"; do
  ...
  {
    printf '# Atlas Cloud CKS binary ISOs — Kubernetes %s — %s\n' "$minor" "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
    printf '# Verify:\n'
    ...
    while IFS= read -r iso; do
      ...
      if ! digest=$(aws "${aws_opts[@]}" s3 cp "s3://${BUCKET_NAME}/cks/${sha_key}" - | awk '{print $1}'); then
  ...
  aws "${aws_opts[@]}" s3 cp "$tmp/${checksum}" "s3://${BUCKET_NAME}/cks/${checksum}" \
      --content-type "text/plain; charset=utf-8" --only-show-errors
  aws "${aws_opts[@]}" s3 cp "$tmp/${checksum}.asc" "s3://${BUCKET_NAME}/cks/${checksum}.asc" \
      --content-type "application/pgp-signature" --only-show-errors
